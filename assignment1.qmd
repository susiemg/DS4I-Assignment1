...
Before the predictive models can be fit, the text needs to be put in a format that can be fed into the algorithm. Three formats were considered: Bag-of-Words, Term frequency-inverse-document-frequency (tf-idf).
...
tf-df is an approach where terms (words) present in a dataset are assigned a weight, where frequently occurring words are downweighted (such as stop words). Two components are calculated:
...
cat("Bag-of-words Accuracy: ", round(sum(diag(pred_test))/sum(pred_test), 3)) 
...
predictions <- predict(fit, test_sona, type = 'class')
pred_test <- table(test_sona$president_13, predictions)
cat("tf-idf Accuracy: ", round(sum(diag(pred_test))/sum(pred_test), 3))  
...
plot(fit, main="Classification tree applied on the trigram model")
text(fit)
```

### Neural Networks

The NN models were fit on the bag-of-words, tf-idf and trigrams models. Below are plots with the training and validation losses and accuracies of each one. A seed was set for reproducibility. The test accuracies were also computed for evaluation.

```{r Neural Networks, warning=FALSE, message=FALSE, echo=FALSE}
#### Bag-of-words ####
set.seed(321)
sample_index <- createDataPartition(bag_of_words$president_13, p = 0.7, list = FALSE)
training_ids <- bag_of_words[sample_index, ] %>%
...
plot(history, main="NN Performance on Bag-of-Words")
...
history <- model %>% fit(train$x, train$y, epochs = 65, 
                         batch_size = 20,  validation_split = 0.1, verbose = 0) 
plot(history, main="NN Performance on tf-idf")
